...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            foreach (var item in categories.Where(x => x.Trim() != ""))// Where is used to filter out empty "" 
            {
                categoryNameBudgetComboBox.Items.Add(item);
            }

            if (categoryNameBudgetComboBox.Items.Count > 0) 
                categoryNameBudgetComboBox.SelectedIndex = 0; //Select the first category as default.
        ...
        private void CalculateButton_Click(object sender, EventArgs e)
        {
            ...
            //Reset the dataGridView1 back color to before checking
    ...
    && dataGridView1.Rows[e.RowIndex].Cells["unitPriceColumn"]?.Value != null)
        ...
        List<BudgetItem> categorysBudgetList = new List<BudgetItem>();
        
        private void addCategoryBudget_Click(object sender, EventArgs e)
        {
            if (categoryNameBudgetComboBox.Text.Trim() == "") 
                return;
            bool CategoryExisted = false;
            foreach (BudgetItem item in categorysBudgetList)
            {
            ...
            if (!CategoryExisted)
            {
                ...
                if (categoryPriceBudgetTextBox.Text != "" && categoryPriceBudgetTextBox.Text != ".")
                {
            ...
            {
                BudgetItem item = categorysBudgetList.Find(x => x.Category.ToLower().Trim() == categoryNameBudgetComboBox.Text.ToLower().Trim());
                if (categoryPriceBudgetTextBox.Text != "" && categoryPriceBudgetTextBox.Text != ".")
                {
                    item.Budget = Convert.ToInt32(categoryPriceBudgetTextBox.Text);                    
                }
            ...
            if (totalItemExisted)
            {
                BudgetItem removelitem = categorysBudgetList.Find(x => x.Category.ToLower().Trim() == "total");
                categorysBudgetList.Remove(removelitem);
            }
            BudgetItem totalitem = new BudgetItem() { Category = "Total", Budget = totalBudget };
            ...
            //{
            //    col.HeaderCell.Style.Font = new Font("Arial", 14F, GraphicsUnit.Pixel);
            //}
        ...
        private void CheckOverBudget()
        {
            ...
            foreach (BudgetItem budgetItem in categorysBudgetList)
            {
                string cateBudgetStr = budgetItem.Category.ToLower().Trim();
                double cateBudgetValue = budgetItem.Budget;
                int cateRealCosts = 0;
                List<int> tempOverbudgetCateRowIndexs = new List<int>();
                foreach (DataGridViewRow dataRow in dataGridView1.Rows)
                {
                    if (dataRow.Cells[0].Value != null && dataRow.Cells[0].Value.ToString().Trim() != ""
                        && dataRow.Cells[0].Value.ToString().ToLower().Trim() == cateBudgetStr)
                    {
                        tempOverbudgetCateRowIndexs.Add(dataRow.Index);
                        cateRealCosts += (dataRow.Cells[4].Value.ToString().Trim() == "") ? 0 : int.Parse(dataRow.Cells[4].Value.ToString());
                    }
                }
                if (cateRealCosts > cateBudgetValue)
                {
                    foreach (int rowindex in tempOverbudgetCateRowIndexs)
                    {
